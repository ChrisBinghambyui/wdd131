...
function recipeTemplate(recipe) {
	return `<article class="recipe-card">
		...
		<div class="recipe-info">
			<div>${tagsTemplate(recipe.tags)}</div>
...
function noResultsTemplate() {
	return `<article class="recipe-card">
		<div class="recipe-info">
			<h2 class="recipe-title">No recipes found</h2>
			<p class="recipe-desc">Try another keyword from a recipe name, description, or tag.</p>
		</div>
	</article>`;
}
...
function renderRecipes(recipeList) {
	if (recipeList.length === 0) {
		recipeContainer.innerHTML = noResultsTemplate();
		return;
	}
