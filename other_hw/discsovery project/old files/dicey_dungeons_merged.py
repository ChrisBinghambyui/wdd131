...
def main():
    ...
    print(" " * 20 + "Merged Best Version")
    print("=" * 70)
    
    # Create player
    player = Player("Hero", max_hp=50, num_dice=3)
    starter_deck = create_starter_deck()
    for card_name, upgrade in starter_deck:
        player.add_card_to_deck(card_name, upgrade)
    
    print(f"\n✓ Player created with {len(player.deck)} cards")
    print(f"  HP: {player.max_hp} | Dice: {player.num_dice}")
    ...
    print(f"✓ Enemy spawned: {enemy.name}")
