...
def print(*args, **kwargs):
  ...
  if args:
    ...
    builtins.print(wrapped_message, **kwargs)
  else:
...
print(Style.BRIGHT + Fore.RED + "K N I G H T" + Style.RESET_ALL + "\nRoll dice individually. If your limit is exceeded, your turn immediately ends. If you match your limit exactly, you get to choose from 3 powerful boons.", delay=0) #on upgrade, the limit increases by 6.
...
print(Style.BRIGHT + Fore.MAGENTA + "R O G U E" + Style.RESET_ALL + "\nYou select a lucky number at the start of each combat. Whenever that number is rolled, you deal 2 damage to your enemy.", delay=0) #on upgrade, add 2 per level
...
class BasicCombatant:
  def __init__(self, name, hp, deck_cards, hand_limit_value, dice_limit_value, intro=None, current_hp=None, max_hp=None):
    self.name = name
    self.intro = intro
...
def _trigger_on_dice_rolled(actor, hide_output_for_blind=False):
  ...
  if actor.status["blind"] > 0 and actor.dice_pool:
    ...
    if hide_output_for_blind:
      print(f"{actor.name} is blinded: {hidden_count} die/dice value(s) are obscured.")
...
def _display_dice_for_player(player_actor):
  ...
  for line_index in range(5):
    print("  ".join(block[line_index] for block in die_blocks), delay=0)
  print("  ".join(labels), delay = 0)
...
def _player_take_turn(player_actor, enemy_actor, auto_player=False):
  ...
  _trigger_on_dice_rolled(player_actor, hide_output_for_blind=True)

  while player_actor.dice_pool and player_actor.cards_in_hand and enemy_actor.hp > 0 and player_actor.hp > 0:
    if auto_player:
      usable_move = None
      ...
      print(f"{enemy_actor.name} HP:{enemy_actor.hp}/{enemy_actor.max_hp} SH:{enemy_actor.shield}")
      ...
      for card_index, hand_card in enumerate(player_actor.cards_in_hand, start=1):
        ...
        print(f"{card_index}) {hand_card.name} | {hand_card.description}{limit_text}")
...
def _enemy_take_turn(enemy_actor, player_actor):
  ...
  _trigger_on_dice_rolled(enemy_actor, hide_output_for_blind=False)
...
def run_basic_combat_loop(current_biome, current_biome_tier=1, player_hp=None, player_max_hp=None, player_deck=None, auto_player=False, legendary_enemy=False):
  """
  Basic combat loop:
  ...
  - Combat ends when enemy dies or player HP drops to 0 (retreat to base camp).
  """
  if current_biome_tier not in [1, 2, 3]:
    print("Invalid biome tier. Please choose 1, 2, or 3.")
    return None
  ...
  if enemy_actor is None:
    print(f"No enemy found for biome {current_biome}, tier {current_biome_tier}.")
    return None
  ...
  if legendary_enemy:
    ...
    print(f"Legendary bonus: HP x1.5 (rounded up), upgraded card: {legendary_upgrade_text}")
  else:
    print("")
    if enemy_actor.intro:
      print(enemy_actor.intro)
    else:
      print(f"A wild {enemy_actor.name} appears in {_format_biome_tier_label(current_biome, current_biome_tier)}!")
  ...
  if enemy_actor.hp <= 0:
    ...
    print(f"{enemy_actor.name} is defeated!")
    return {
      "result": "victory",
  ...
  _reset_combat_state(player_actor)
  _reset_combat_state(enemy_actor)
  print("You were reduced to 0 HP or less and retreat to base camp.")
  return {
    "result": "retreat",
    "player_hp": player_actor.hp,
    "enemy": enemy_actor.name,
...
def adventuring_loop(current_biome, current_biome_tier, max_player_health):
  ...
  while current_player_health > 0:
    ...
    for option_index, (biome_value, tier_value) in enumerate(sorted_unlocked, start=1):
      print(f"{option_index}) {_format_biome_tier_label(biome_value, tier_value)}")
...
def run_tutorial(seed=7):
  ...
  print("\n=== Tutorial Combat ===")
  print(f"{player_name} HP: {player_hp} | {enemy['name']} HP: {enemy['hp']}")
  ...
  while player_hp > 0 and enemy["hp"] > 0:
    ...
    while hand and dice_pool and enemy["hp"] > 0:
      ...
      if chosen_die_value < chosen_card["cost"]:
        print("That die is too low for this card. Pick another card.")
        continue
      ...
      if enemy["hp"] > 0:
        print(f"Current HP -> {player_name}: {player_hp} | {enemy['name']}: {enemy['hp']}")
    ...
    print(f"End round -> {player_name}: {max(0, player_hp)} HP | {enemy['name']}: {max(0, enemy['hp'])} HP")
    round_number += 1
...
def _debug_prompt_int(prompt_text, default_value, minimum_value=1, maximum_value=None):
  ...
  if parsed < minimum_value:
    print(f"Value must be at least {minimum_value}. Keeping previous value.")
    return default_value
  if maximum_value is not None and parsed > maximum_value:
    print(f"Value must be at most {maximum_value}. Keeping previous value.")
    return default_value
  return parsed
...
def _debug_fill_deck_by_family_tier(current_deck):
  ...
  while True:
    ...
    if family_code is None:
      print("Unknown family. Try a valid name or code from 00 to 80.")
      continue

    family_data = family_lookup[family_code]
    print(f"Selected family {family_code} - {family_data['name']}")
    ...
    if tier_code is None:
      print("Invalid tier input.")
      continue
    if tier_code not in family_data["tiers"]:
      print(f"Tier {tier_code} is not defined for this family.")
      continue
    ...
    print(f"Added {copies}x {chosen_card.name}.")

  print(f"Deck now has {len(current_deck)} card(s).")
  return current_deck
...
def run_debug_battle_lab():
  ...
  print("\n=== DEBUG BATTLE LAB ===")
  print("Run infinite test battles and tweak stats between fights.\n")
  ...
  while True:
    ...
    if choice == 1:
      ...
      if encounter_result is None:
        print("Battle could not start with current settings.")
        continue
      ...
      if debug_current_hp <= 0:
        print("You were defeated. Resetting current HP to max for the next test.")
      ...
      if not debug_player_deck:
        ...
        print("Deck was empty; restored fallback deck.")
    elif choice == 9:
      debug_current_hp = debug_max_hp
      print(f"Current HP reset to {debug_current_hp}/{debug_max_hp}.")
    else:
      play = "quit"
...
while play!="quit":
  ...
  if x == 1:
    print("The sea air blows around you as you watch the waves break against your ship. You hoped to make it to your destination without any storms, but it seems luck is not on your side. The sea churns and grows violent as you sail under the rolling black clouds. You've never seen clouds quite so dark. If you weren't on the open ocean, you'd think it was smoke.\n", delay=5)
    print("The deck lurches, as though colliding with a rock, but as the captain orders the sailors about, you take a peek over the side and find nothing that could have caused the movement, just the shadows of marine life zipping around the beleagered boat. But they're too fast. A loud splash sounds, and you whip around to see a silhouette behind you. A sailor you've never met stands, sword drawn, dripping wet. It opens it's mouth, letting out a low whistle, before raising its blade and charging you.\n", delay=2.5)
    result = run_tutorial()
    if result == "victory":
        print("You stand over the defeated pirate. The strange man looks up at you, then begins to... melt. He dissappears, seeping into the cracks in the boards of the ship, leaving you alone. Truly alone, because as you look around, you can find none of the other sailors manning the ship. The storm has calmed, but the clouds still fill the sky. The sea is eerily still. Then you feel something. A terrible rumbling, growing all around you. The ship explodes beneath your feet, and you are sent sailing into the air. You manage to get a look at a massive creature, a sea serpent of some kind, emerging from where you were just standing, before you collide with the water and everything goes black", delay=10)
    elif result == "defeat":
        print("You lie on your back, defeated. You try to inch away from the strange pirate, but with the rail behind you and his blade before you, you have nowhere to go. A rumbling sounds all around the ship, and the pirate finally takes it's unearthly dark eyes off you as it looks around frantically. You take the chance to scramble to the side and put some distance between you and your foe. You grab a stry chunk of wood as a primitive defensive club, but when you turn around you witness the pirate diving off the side of the ship. You rush to the side to see where he went, but despite searching for a few minutes you see nothing. You turn around and are faced with something infinitely worse. A massive sea serpent, 300, no 400 feet tall at least, looms above your ship, huge glowing blue eyes focused unblinking at you. Your head begins to swim, and you collapse against the rail for support. The eyes seem to grow in both size and intensity, and soon you can see nothing else. You try to turn, but are unable to escape the blue light. Soon, your mind collapses, and the world flees from you.", delay=10)
  print("...")
  print("...")
  print("...")
  print("...")
  print("You remember the smell of the sea...", delay=1.5)
  print("...")
  print("...")
  print("You remember the swaying of the ship...", delay=1.5)
  print("...")
  print("...")
  print("You remember..."+Style.BRIGHT+Fore.CYAN+ "lightning...", delay=1.5)
  print(Style.RESET_ALL + "...")
  print(Style.DIM+Fore.CYAN+"Wind...", delay=1.5)
  print(Style.RESET_ALL + "...")
  print("And then the icy grip of the sea...", delay=1.5)
  print("...")
  print("...")
  print("...")
  print("...")
  x=input(Style.BRIGHT + Fore.RED + "GET UP?" + Style.RESET_ALL + "\n\t>")
  print("As life slowly awakens in your head, you feel coarse sand beneath you and the gentle nudging of waves against your leg. Your first attempt to move your arms fails you, but after a few moments you're able to place them underneath you ans shakily stand. You see the wide golden expanse of plains ahead.\n", delay=2.5)
  print("You brush off some of the sand and turn to see what's left of the crash. However...\n", delay=2.5)
  print("You are not on a seaside beach or some lonely island.\n", delay=2.5)
  print("Before you is a lake. A large lake, to be sure, but you scan the bredth of it easily without squinting.\n", delay=2.5)
  print("The only connections this lake has are a small creek trickling in from the north, and a slow, shallow stream exiting it to the west.\n", delay=2.5)
  print("Certainly confusing, but you have more things to worry about now. Some of the ship you were on seems to have made it to this strange lakeside alongside you, but you see no evidence of other survivors. You spend the rest of the day collecting debris and searching wreckage. You manage to scrounge together some remnants of what was probably the sail, and set up a simple shelter for the night.\n", delay=2.5)
  print("The night, thankfully, passes quickly. As you make your way outside in the morning, you see small tracks left by some evening passerby, not far from your makeshift tent. Just the thought of something else alive makes you realize how hungry you are. Maybe you can find some animal to eat, or, if you're lucky, a tavern. You slide a small metal shard into your pocket to use as a knife, and break off an unrotted piece of wood as a primitive club.\n")
  print("By midday, you are travelling through those beautiful golden stalks. This looks like farmland. Hopefully it's farmer is nearby. You hear a rustling to your side, and turn to investigate. You'd hoped to find help, but your luck has betrayed you once again.\n")
  poison_counter = 0
  dice = []
  cards_in_hand = []
  ...
  adventuring_loop(1, 1, max_player_health)
  ...
  while play != "quit":
    if first_time == False:
      if result == "defeat":
        first_time = True
        print("You stagger to relative safety before collapsing on the ground. That last fight got you good. Luckily, you had some straps leftover from the sail that you use to make a makeshift bandage.\n", delay=5)
        print("As you finish wrapping, you think you hear the sound of carriage wheels. You must be getting delirious. Regardless, you limp towards the sound, hoping beyond hope to find some sort of aid.\n", delay=5)
        print("You stumble out onto a worn dirt road. Slowly getting further and further is a wagon. You weakly croak, wave your arms, but you seem unable to get the attention of the wagon driver.\n", delay=5)
        print("With all your remaining strength you let out a shout, but the effort leaves you nearly breathless, and you fall to your knees. As your vision fades. You can see the wagon slow, and a silhouette hop off and rush towards you.\n", delay=5)
        print("What seems to be either only a few moments later, or a dew decades, you are able to again open your eyes. You are lying in a bed, in a room faintly lit by torchlight. You try to sit up, but wince in pain.\n", delay=5)
        print("Your wounds are bandaged. A strange salve is on a scratch on your arm. It has an unpleasant smell, but you can't feel any pain from it. As you pat yourself down, inspecting your wrappings, a voice clears from the corner of the room.\n", delay=5)
        print('"You took quite the beating, stranger," says the voice. It is somehow both very deep, yet still soft. A silhouette forms and steps into the light, and you are finally able to get a good look at your savior.\n', delay=5)
        print("You recognize them as a bovari, one of the ox-folk. The bovari stands about as tall as you, but her horns curl up another foot. Her impressive arms split at the elbows, ending in four powerful hands, the two left ones holding small vials.\n", delay=5)
        print('"Not many travellers pass through here," she says, wiping away the salve from your arm before applying a new coat. "We can\'t let the few that do make it croak on us, can we?"\n', delay=5)
        print(f'As she works, you ask for her name. "Thessa", she says. "What\'s yours?" "{charname}," you reply. "Don\'t know where you were headed, but you\'ve made it to Ascus." She says this like it\'s supposed to mean something to you, but you\'ve never heard the name before.\n', delay=5)
        print('After a few moments tending to your wounds, she helps you up. The salve is working miracles, as you\'re able to make it out the door and down the stairs without much help.\n', delay=5)
        print("You're in a modest tavern, filled with ratmen. The Naukin are a reclusive people, and they validate that stereotype, eyeing you and hushing their conversations as you enter the room.\n", delay=5)
        print('Thessa leads you to the bar counter and helps you into a seat, before going behind and fixing a few drinks. "Don\'t mind them, they\'re always wary of outsiders." She slides you a drink, before getting to work on another for another customer.\n', delay=5)
        print("You down the drink, a warm, earthy beverage that fills your stomach. \"What do I owe you?\", you ask, sheepishly patting youreself down for a coinpurse you both know isn't there.\n", delay=5)
        print("The woman laughs. \"A stack of clean dishes is a good start\", she says, gesturing to the kitchen.\n", delay=5)
      if result == "victory":
        print("You pat yourself down, surprised you made it out with as few scratches as you did. Looking around, you spot a faint trail of smoke in the distance, almost like from a chimney. You decide to head towards it.\n", delay=5)
        print("After about another hour of hiking through wheat fields, yyou can finally see a small village in the distance. A few houses and other buildings are loosely scattered around a sturdy-looking tavern, and the scent of fresh bread drifts towards your nose.\n", delay=5)
        print("As you enter the village, the citizens grow quiet, and you can feel all eyes on you. The denizens seem to be mostly Naukin, rat-folk known for being extremely reclusive and secluded by nature.\n", delay=5)
        print("The tavern greets you with mostly the same reaction, with one notable excepetion: a large friendly Bovari bartender. The ox-folk warmly gestures you over, greeting you with a drink ready before you even sit down.\n", delay=5)
        print('"Welcome to Ascus, traveller," says the bartender. "My name is Thessa. We don\'t get many visitors here, so don\'t mind the strange looks. Took them a couple years to get used to me," she chuckles.\n', delay=5)
        print('"What can I do you for?", Thessa asks, her four hands cleaning the counter and pouring drinks with remarkable dexterity.')