...
print(Style.BRIGHT + Fore.MAGENTA + "R O G U E" + Style.RESET_ALL + "\nYou select a lucky number at the start of each combat. Whenever that number is rolled, you deal 2 damage to your enemy.") #on upgrade, add 2 per level
...
class BasicCombatant:
  def __init__(self, name, hp, deck_cards, hand_limit_value, dice_limit_value):
    self.name = name
    self.max_hp = hp
    self.hp = hp
    self.shield = 0
...
def _resolve_card_use(source, target, card_obj, die_value):
  ...
  if not effect_triggers_now:
    ...
    return True, progress_text, False
  ...
  action_text = f"{source.name} uses {card_obj.name} with a {die_value} for {hp_damage_done} HP damage."
...
def _display_dice_for_player(player_actor):
  ...
  for index, value in enumerate(player_actor.dice_pool, start=1):
    if (index - 1) in player_actor.hidden_die_indices:
      visible_parts.append(f"{index}:?")
    else:
      visible_parts.append(f"{index}:{value}")
  print("Dice:", ", ".join(visible_parts) if visible_parts else "(none)")
  ...
  while player_actor.dice_pool and player_actor.cards_in_hand and enemy_actor.hp > 0 and player_actor.hp > 0:
    if auto_player:
      ...
      for card_index, hand_card in enumerate(player_actor.cards_in_hand, start=1):
        print(f"{card_index}) {hand_card.name} | {hand_card.description}")
...
def run_basic_combat_loop(current_biome, current_biome_tier=1, player_hp=None, player_deck=None, auto_player=False, legendary_enemy=False):
  """
  Basic combat loop:
...
def run_tutorial(seed=7):
  ...
  while player_hp > 0 and enemy["hp"] > 0:
    ...
    print(f"You roll dice: {dice_pool}")
    ...
    while hand and dice_pool and enemy["hp"] > 0:
      ...
      for index, die_value in enumerate(dice_pool, start=1):
        print(f"  {index}) {die_value}")
...
while play!="quit":
   ...
   print("...")
   print("...")
   x=get_valid_input(Style.BRIGHT + Fore.RED + "GET UP?" + Style.RESET_ALL + "\n\t>", [1])
